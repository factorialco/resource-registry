...
module ResourceRegistry
  ...
  class Resource < T::Struct
    ...
    def identifier
      @identifier ||=
        T.let(
          :"#{namespace.underscore}.#{name.to_s.underscore}",
          T.nilable(Symbol)
        )
    end
    ...
    def name
      @name ||=
        T.let(resource_name.underscore.singularize.to_sym, T.nilable(Symbol))
    end
    ...
    def repository
      ...
      if repository_klass.nil?
        raise ArgumentError,
              "Repository #{repository_raw} not found, did you misspell it?"
      end
    ...
    def build_dto(verb, **parameters)
      ...
      if verb_info.nil?
        raise VerbNotFound, "#{name} resource doesn't have a ##{verb} verb"
      end
    ...
    sig do
      params(key: Symbol).returns(T.nilable(Capabilities::CapabilityConfig))
    end
    ...
    def capability!(feature)
      ...
      raise ArgumentError,
            "Resource #{name} does not have #{feature} capability"
    end
    ...
    def self.load(spec, configuration: ResourceRegistry.configuration)
      repository =
        (
          if spec["repository"].is_a?(Symbol)
            spec["repository"].to_s
          else
            spec["repository"]
          end
        )
